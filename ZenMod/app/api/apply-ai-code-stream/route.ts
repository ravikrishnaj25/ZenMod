  ...
  try {
    ...
    if (!sandbox && sandboxId) {
      ...
      try {
        ...
        console.warn(`[apply-ai-code-stream] Failed to reconnect to sandbox ${sandboxId}, creating NEW sandbox:`, reconnectError);

        // Attempt to create a NEW sandbox instead of failing
        try {
          sandbox = await Sandbox.create({
            apiKey: process.env.E2B_API_KEY,
            timeoutMs: 900 * 1000 // 15 minutes in ms
          });

          console.log(`[apply-ai-code-stream] Created new replacement sandbox: ${sandbox.sandboxId}`);

          // Store globally
          global.activeSandbox = sandbox;

          // Reset file cache and existing files since it's a new sandbox
          global.existingFiles = new Set<string>();
          ...
          // We'll continue with the new sandbox
        } catch (createError) {
          console.error(`[apply-ai-code-stream] Critical failure: Could not reconnect OR create new sandbox:`, createError);
          return NextResponse.json({
            success: false,
            error: `Sandbox error: ${(createError as Error).message}`,
            results: {
              filesCreated: [],
              packagesInstalled: [],
              commandsExecuted: [],
              errors: [`Reconnection failed: ${(reconnectError as Error).message}`, `Creation failed: ${(createError as Error).message}`]
            },
            explanation: parsed.explanation,
            structure: parsed.structure,
            parsedFiles: parsed.files,
            message: `Could not apply code - sandbox is unavailable.`
          });
        }
      }
    }
...
print(f"File written: ${fullPath}")
        ...
        await sendProgress({
          type: 'complete',
          results,
          sandboxId: (sandboxInstance as any).sandboxId,
          explanation: parsed.explanation,
          structure: parsed.structure,
